...
function recognizeFile(file){
	...
	worker.recognize(file,
		...
		.then(function(data){
			...
			// console.log(data.words.text)
			console.log(data.words.length)

			// let chars = data.words.split
			...
			{
				if ((data.words[i]).text.length == 4)
				{
					if (Number(data.words[i].text))
					{
						console.log((data.words[i]).length)
						
						str += data.words[i].text
